...
$( function(){
	$("#moduloRicerca").on('submit', function(event){
		...
		$.post(
			urlbase() + 'ricerche.php/album-semplice',
			modulo
		)
		.done(function(json_ret){
			...
			if (typeof json_in['error'] !== 'undefined'){
				...
				json_in['data'].forEach(element => {
					...
					album_html_item = '<tr>'
					+'<td><a href="'+urlbase()+'album.php/leggi/'+element.record_id+'" target="_blank" ><i class="bi bi-archive" style="font-size:1rem;color: #'+element.tinta_rgb+'"></i></a>&nbsp;</td>'
					+'<td><a href="'+urlbase()+'album.php/leggi/'+element.record_id+'" target="_blank" style="text-decoration: none;"><span style="font-size:1rem;color: #'+element.tinta_rgb+'">'+element.titolo_album+'</span></a><br>'
		...
		$.post(
			urlbase()+'ricerche.php/fotografie-semplice',
			modulo
		)
		.done(function(json_ret){
			var json_in = $.parseJSON(json_ret);
			...
			if (typeof json_in['error'] !== 'undefined'){
				...
				json_in['data'].forEach(element => {
					...
					+ '<a href="'+urlbase()+'fotografie.php/leggi/'+element.record_id+'" '
					+ 'title="'+element.titolo_fotografia+' " target="_blank" >'
					+ '<img src="'+urlbase()+element.percorso_completo+'" '
