 ...
 * /fotografie.php/leggi/{fotografia_id}
 *   carica la scheda della fotografia e 
 *   mostra la pagina 
 * 
 * /fotografie.php/richiesta/{fotografia_id}
 *   carica in tabella la richiesta e riespone la fotografia 
 * 
 * /fotografie.php/precedente/{fotografia_id}
 *   carica la scheda della fotografia precedente 
 * 
 * /fotografie.php/seguente/{fotografia_id}
 ...
 * /fotografie.php/modifica-dettaglio/{dettaglio_id}?f=fotografia_id
 ...
 * 
 * /fotografie.php/carica_dettaglio/{fotografia_id}
 *   prepara e mostra la pagina di aggiunta dettaglio 
 * 
 * /fotografie.php/aggiungi-dettaglio/{fotografia_id}
 ...
 * 
 * TODO /fotografie.php/cancella/{fotografia_id}
...
$fotografia_id = $pezzi['operazioni'][1];
$dettaglio_id  = $pezzi['operazioni'][1]; // lo stesso ma con altro significato 
if (!is_numeric($fotografia_id)){
	http_response_code(404); // TODO sostituire con il codice errore parametro invalido 
	echo '<pre style="color: red;"><strong>Manca un id</strong></pre>'."\n";
	exit(1);
}
if ($richiesta != 'carica-dettagli-da-fotografia' && ($fotografia_id)<1 ){
	http_response_code(404); // TODO sostituire con il codice errore parametro invalido 
	echo '<pre style="color: red;"><strong>Manca un id</strong></pre>'."\n";
	exit(1);
}
...
if ($richiesta === 'precedente'){
	$fotografia_precedente = leggi_fotografia_precedente($fotografia_id);
	// $_SESSION['messaggio]
...
if ($richiesta === 'seguente'){
	$fotografia_seguente = leggi_fotografia_seguente($fotografia_id);
	// $_SESSION['messaggio]
...
if ($richiesta === 'richiesta'){
	carica_richiesta_fotografie_per_id($fotografia_id);
	// $_SESSION['messaggio]
	leggi_fotografie_per_id($fotografia_id);
	exit(0);
}
...
if ($richiesta === 'carica-dettaglio'){
	aggiungi_fotografie_dettagli_da_modulo($fotografia_id, []);
	exit(0);
}
...
if ($richiesta === 'aggiungi-dettaglio' && isset($_POST['aggiungi_dettaglio'])){
	aggiungi_fotografie_dettagli_da_modulo($fotografia_id, $_POST);
	exit(0);
}
