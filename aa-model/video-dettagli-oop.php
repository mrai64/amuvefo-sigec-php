<?php
/**
 * @source /aa-model/video-dettagli-oop.php
 * @author Massimo Rainato <maxrainato@libero.it>
 *
 * Classe VideoDettagli
 *
 *	dipendenze: DatabaseHandler connessione archivio PDO
 *	dipendenze: Video tabella padre
 *	dipendenze: Descrizioni tabella di lunghi testi
 *
 * Per ogni video i dati associati non dovrebbero essere
...
Class VideoDettagli {
	...
	public function set_record_id_padre( int $record_id_padre ){
		...
		// TODO: si può aggiungere un check se presente in tabella_padre Video
	}
	...
	public function set_consultatore_id( int $consultatore_id ){
		...
		// TODO: si può aggiungere un check se presente in tabella calendario_consultatori
	}
	...
	public function aggiungi( array $campi = []){
		...
		if (!isset($campi['record_id_padre'])){
			$ret = [
				"error"=> true,
				"message" => __CLASS__ . ' ' . __FUNCTION__
				. " VideoDettagli::record_id_padre deve essere intero unsigned. "
			];
			return $ret;
		}
		...
		if (!isset($campi['chiave'])){
			$ret = [
				"error"=> true,
				"message" => __CLASS__ . ' ' . __FUNCTION__
				. " VideoDettagli::chiave deve essere valorizzato. "
			];
			return $ret;
		}
		...
		if (!isset($campi['valore'])){
			$ret = [
				"error"=> true,
				"message" => __CLASS__ . ' ' . __FUNCTION__
				. " VideoDettagli::valore deve essere valorizzato. O lo volete cancellare? "
			];
			return $ret;
		}
